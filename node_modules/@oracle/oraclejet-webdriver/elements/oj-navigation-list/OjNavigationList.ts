...
/**
 * This is the component WebElement for oj-navigation-list. Retrieve an instance of the class
 * using its convenience function:
 * ```javascript
 * await ojNavigationList(driver, By.css('oj-navigation-list'))
 * ```
 */
...
Use the ExpandAllKeySet class to expand all items.
  ...
  private _getExpansionTree(pathsToBeExpanded:Array<Array<number>> = []):any {
    let children:any, expansionTree:any = {};
...
Use the ExpandAllKeySet class to expand all items.
  ...
  private async _getItemByIndex(index: number, parent?: WebElement):Promise<WebElement> {
    ...
    try {
      if (!parent)
        item = await this.findElement(By.css('.oj-navigationlist-element > .oj-navigationlist-item-element:nth-child(' + index + ')'))
      else {
        item = await parent.findElement(By.css('.oj-navigationlist-group > .oj-navigationlist-item-element:nth-child(' + index + ')'))
      }
    } catch (e) {
