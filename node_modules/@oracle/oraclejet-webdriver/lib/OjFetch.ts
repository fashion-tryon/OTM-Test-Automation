...
export interface ClientFetch {
  fetch(driver: WebDriver, requestInfo:MarshalableRequestInfo, responseInfo:MarshalableResponseInfo, init?: MarshalableRequestInit) : Promise<MarshaledResponse>;
};
...
export class OjFetch implements ClientFetch {
   async fetch(driver: WebDriver, requestInfo:MarshalableRequestInfo, responseInfo:MarshalableResponseInfo, 
    init?: MarshalableRequestInit) : Promise<MarshaledResponse> 
   { // webdriver's async call marshalls objects on callback, so we have to unmarshall back into promise
     ...
     const remoteScript =
     `var input=arguments[0], info=arguments[1], init=arguments[2],  callback = arguments[arguments.length - 1], resp = {};
      fetch(input,init)
      ...
      try {
        ...
        let resp:AsyncResponse = await driver.executeAsyncScript(remoteScript, requestInfo, responseInfo, fetchInit) as AsyncResponse;
        ...
        if(resp.error){
          return new Promise<MarshaledResponse>((resolve,reject) => {reject('Response encountered error: ' + resp.error  
        + "(requestInfo = " + rInfo + ", response = " + JSON.stringify(resp,null,2) +  " status = " + resp.status + ")");});
        }
        retResults = new Promise<MarshaledResponse>((resolve,reject) => {resolve(myResponse);});        
      } catch(err) {
        retResults = new Promise<MarshaledResponse>((resolve,reject) => {reject('Unable to read Response: ' + err 
        + "(requestInfo = " + rInfo + ")");});
      };
