...
class WebElementRegistry {
  ...
  public static register(
    ...
    WebElementRegistry.tagToCtorMap[tagName] = ctor;
  }
  /**
  ...
  public static get(
    ...
    return WebElementRegistry.tagToCtorMap[tagName];
  }
  /**
...
WebDriver.prototype.findElement = wrapFindElement(
  WebDriver.prototype.findElement
);
WebDriver.prototype.findElements = wrapFindElements(
  WebDriver.prototype.findElements
);

WebElement.prototype.findElement = wrapFindElement(
  WebElement.prototype.findElement
);
WebElement.prototype.findElements = wrapFindElements(
  WebElement.prototype.findElements
);

WebDriver.prototype.executeScript = wrapExecuteScript(
  WebDriver.prototype.executeScript
);
WebDriver.prototype.executeAsyncScript = wrapExecuteScript(
...
function wrapFindElement(origFunc: (locator: Locator) => WebElementPromise) {
  return function findElement(locator: Locator): WebElementPromise {
    ...
    return new WebElementPromise(driver, new Promise<WebElement>(async (resolve, reject) => {
      try {
 ...
 * @private
 */
function wrapFindElements(
  origFunc: (locator: Locator) => Promise<WebElement[]>
) {
  ...
  return async function<T>(...args: any[]): Promise<T> {
    try {
      ...
      if (ex instanceof error.StaleElementReferenceError) {
        ...
        console.warn(
          `Trapped stale element executing script on ${locators}`
        );
