...
/**
 * This is the component WebElement for oj-data-grid. Retrieve an instance of the class
 * using its convenience function:
 * ```javascript
 * await ojDataGrid(driver, By.css('oj-data-grid'))
 * ```
 */
export class OjDataGrid extends OjDataGridBase {
  ...
  async getHeaderContents(subId: headerSubId, by: By): Promise<Array<WebElement>|null> {
    ...
    let headers = await this.findElements(By.css('.oj-datagrid-' + axis.replace('End', '-end') + '-header-cell'));
  ...
  async getLabelContents(subId: labelSubId, by: By): Promise<Array<WebElement>|null> {
    ...
    let labels = await this.findElements(By.css('.oj-datagrid-' + axis.replace('End', '-end') + '-label'));
