 ...
 */
export async function requeryElement<T extends OjWebElement>(
  el: T
): Promise<T> {
  ...
  if (
    el.constructor !== requeried.constructor ||
    locators.tagName !== reTagName
  ) {
    throw Error(
      `Expected to find <${locators.tagName}> but got <${reTagName}>`
    );
  }
  return requeried;
}
