...
//export screenshotManager module
...
async function checkVersions(driver: WebDriver) {
  ...
  if (
    ...
    !(version === '1.0.0' && localRegex.test(rev)) &&
    // or version mismatch
...
const ojwd = {
  ...
  async get(driver: WebDriver, url: string) {
    ...
    try {
      ...
      if (ex instanceof error.TimeoutError || ex instanceof error.ScriptTimeoutError) {
        const openStates = await driver.executeAsyncScript<string[]>(`
   ...
   * @param {string} init.method: The request method, e.g., GET, POST. Note that the Origin header is not set on Fetch requests with a method of HEAD or GET (this behavior was corrected in Firefox 65 — seebug 1508661).
   ...
   * the observable.
   * ```javascript
   * new JetExpecation(By.id('view-container'), 'firstName()')
   * ```
   *
   ...
   */
  async assertViewModelValue(driver: WebDriver, vmLocator: Locator, expectation: Expectation): Promise<any> {
