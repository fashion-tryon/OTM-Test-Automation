...
/**
 * This is the component WebElement for oj-checkboxset. Retrieve an instance of the class
 * using its convenience function:
 * ```javascript
 * await ojCheckboxset(driver, By.css('oj-checkboxset'))
 * ```
 */
export class OjCheckboxset extends OjCheckboxsetBase {
  ...
  async changeValue(value: any[]) {
    ...
    if (!(await this.isInteractable())) {
      ...
      return Promise.reject(new error.ElementNotInteractableError("OjCheckboxset: Value cannot be changed when the control is disabled or readonly"));
    }
    ...
    for (let i = 0, len = value.length; i < len; i++) {
      ...
      if (input == null) {
        ...
        return Promise.reject(new error.InvalidArgumentError(`OjCheckboxset: ${value[i]} is not a valid value.`));
      }
