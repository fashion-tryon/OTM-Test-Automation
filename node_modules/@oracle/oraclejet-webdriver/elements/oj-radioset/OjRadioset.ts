...
/**
 * This is the component WebElement for oj-radioset. Retrieve an instance of the class
 * using its convenience function:
 * ```javascript
 * await ojRadioset(driver, By.css('oj-radioset'))
 * ```
 */
export class OjRadioset extends OjRadiosetBase {
  ...
  async changeValue(value: any) {
    ...
    if (!(await this.isInteractable())) {
      ...
      return Promise.reject(new error.ElementNotInteractableError("ojRadioset: Value cannot be changed when the control is disabled or readonly"));
    }
    ...
    if (input == null) {
      ...
      // oj-radioset element
      return Promise.reject(new error.InvalidArgumentError(`ojRadioset: ${value} is not a valid value.`));
    }
